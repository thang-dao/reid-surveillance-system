...
def iresnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        #                                                    root=default_cache_path)))
        pretrain_dict = torch.load(download_from_url(model_urls['iresnet50'],
                                                           root=default_cache_path))
        pretrain_dict = {
            k: v
...
def iresnet101(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        #                                                    root=default_cache_path)))
        pretrain_dict = torch.load(download_from_url(model_urls['iresnet101'],
                                                           root=default_cache_path))
        pretrain_dict = {
            k: v
            for k, v in pretrain_dict.items()
...
if __name__ == '__main__':
    model = iresnet101(pretrained=True)
